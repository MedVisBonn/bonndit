...
#
		...
		if self.store_loss:
			...
			for i in range(self._model.num_tensors):
				...
				self.mean[i, 2] *= -1
				self.mean[i, 4] *= -1
				self._model.dipy_v[0] *= self._model.rank_1_rh_o4[0]
				self._model.dipy_v[3] *= self._model.rank_1_rh_o4[1]
				self._model.dipy_v[10] *= self._model.rank_1_rh_o4[2]
...
cdef class UKFBingham(UKF):
	...
	cdef int select_next_dir(self, int info, double[:] old_dir):
		...
		for i in range(self._model.num_tensors):
			...
			self.mean[6*i + 2] = min(max(self.mean[6 * i + 2], log(0.2)), self.mean[6 * i + 1])
...
##
#				cblas_dscal(self.res.shape[0], 0, &self.res[0], 1)
#				self._model1.(exp(self.mean[j,0]), &self.res[0], 4)
#				self.res[0] *= self._model1.rank_1_rh_o4[0]
#				self.res[3] *= self._model1.rank_1_rh_o4[1]
#				self.res[10] *= self._model1.rank_1_rh_o4[2]
#				self.mean[j,2] *= -1
